...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(f"https://appteste-557e1-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://appteste-557e1-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",data=info)

        self.mudar_tela("ajustespage")

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://appteste-557e1-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f"https://appteste-557e1-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",data=info)
    ...
    def adicionar_venda(self):
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade)==float):
            ...
            requests.post(f"https://appteste-557e1-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}",data=info)
            ...
            requisicao = requests.get(f"https://appteste-557e1-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
            total_vendas = float(requisicao.json())
            total_vendas += preco
            info = f'{{"total_vendas": "{total_vendas}"}}'
            requests.patch(f"https://appteste-557e1-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                           data=info)
            homepage = self.root.ids["homepage"]
            homepage.ids["label_total_vendas"].text = f"[color=#000000]Total de Vendas:[/color] [b]R${total_vendas}[/b]"
            self.mudar_tela("homepage")
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(f'https://appteste-557e1-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()

        # preencher foto de perfil
        foto_perfil = self.root.ids["foto_perfil"]
        foto_perfil.source = f"icones/hash.png"
